...
var chestUpload = function (inputFile, server, port, response, callback) {
  var fs             = require ('fs');
  ...
  socket.on ('connect', function () {
    response.log.verb ('connected to the chest server');
    ...
    socket.on ('disconnect', function () {
      ...
      response.log.verb ('disconnected from the chest server');
    ...
    socket.on ('registered', function (error) {
      ...
      response.log.info ('begin file upload');
      ...
      var reqFile = request (options, function (error, response, body) {
        ...
        if (body) {
          response.log.verb (body);
        }
      });

      stream.on ('end', function () {
        response.log.info ('transfer average speed: %d [Mbps]',
                     parseInt (progressSpeed * 8 / 1000) / 1000);
        response.log.info ('the uploaded file is synchronizing in the repository...');
      });
...
exports.upload = function (file, chestConfig, response, callback) {
  try {
    chestUpload (file,
                 chestConfig.host,
                 chestConfig.port,
